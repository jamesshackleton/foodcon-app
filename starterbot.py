...
FOODCON_BOT_VERSION = "v0.1.8 - Early Alpha - Now with case insensitive commands! GIT TEST11"
...
def handle_command(command, channel):
    ...
    if command.casefold().startswith(FOODCON_STATUS_COMMAND): #& foodcon_level is '1' or '2' or '3' or '4' or '5':
        try:
            response = "WE ARE CURRENTLY AT *FOODCON " +foodcon_level+ "*"
        except NameError:
            response = "The FOODCON level must be set before you can use this command. Try 'set `value`'"



    if command.casefold().startswith(FOODCON_HELP_COMMAND):
        response = "FOODCON App - " +FOODCON_BOT_VERSION+ "\n This is currently a primitive bot, all commands must be '@foodcon `command`'\n Commands: \n \n *" +FOODCON_STATUS_COMMAND+ \
                                                      "* - Returns the current FOODCON level. \n *" +FOODCON_SET_COMMAND+ "* `1`, `2`, `3`, `4` or `5` - Sets the FOODCON level " \
